import CommentAddForm from "@/app/ui/recruits/CommentAddForm";
...
export default async function Page(props: PageProps) {
  ...
  if (access.status === "signed-out") {
    redirect(loginRedirectFor(`/recruits/${id}/add-comment`));
  }
  if (access.status === "closed") {
    return <RecruitsClosed title="ADD COMMENT" />;
  }
    ...
    <div className="flex flex-col min-h-screen bg-black text-white">
      ...
      <div className="relative w-full flex flex-col items-center">
        ...
        <div className="mt-[-5rem] px-4 max-w-lg mx-auto w-full">
          <CommentAddForm recruitId={id} brotherId={brotherId} />
        </div>
      </div>
    </div>
  );
